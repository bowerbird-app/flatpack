...
- Closing the sidebar no longer restyles icons when the width transition ends. Labels shrink and icons ease to center on `--duration-slow`, so the rail does not shudder into a second pose.
...
- Do not restyle sidebar icons when the width transition ends. Center them with the width ease.
