...
- Searchable Select, Combobox, and the FlatPack date picker open and close like Popover: opacity plus a 4px offset on `--duration-base` / `--easing-enter` / `--easing-exit`. They share `playOverlayEnter` / `playOverlayExit`. Reduced motion skips the offset and hides on the next turn.
...
- Hosts that snap `hidden` on searchable Select, Combobox, or date picker panels should use `playOverlayEnter` / `playOverlayExit` from `controllers/flat_pack/reduced_motion`. Hide is delayed by `--duration-base` (0ms under reduced motion). Date picker still sets `display: none` after the exit finishes because the panel also uses `md:flex`.
