...
module FlatPack
  module EmptyState
    class Component < FlatPack::BaseComponent
      ...
      ICONS = {
        inbox: <<~SVG.html_safe,
          <svg class="w-12 h-12 text-[var(--color-text-muted)]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      def render_title
        content_tag(:h3, @title, class: "text-lg font-semibold text-[var(--color-text)] mb-2")
      end

      def render_description
        return nil unless @description

        content_tag(:p, @description, class: "text-sm text-[var(--color-text-muted)] max-w-md mb-6")
      end
