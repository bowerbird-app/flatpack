# frozen_string_literal: true

module FlatPack
  module Toast
    class Component < FlatPack::BaseComponent
      # Tailwind CSS scanning requires these classes to be present as string literals.
      # DO NOT REMOVE - These duplicates ensure CSS generation:
      # "border-[var(--toast-info-border-color)]" "bg-[var(--toast-info-background-color)]" "text-[var(--toast-info-text-color)]" "text-[var(--toast-info-icon-color)]"
      # "border-[var(--toast-success-border-color)]" "bg-[var(--toast-success-background-color)]" "text-[var(--toast-success-text-color)]" "text-[var(--toast-success-icon-color)]"
      # "border-[var(--toast-warning-border-color)]" "bg-[var(--toast-warning-background-color)]" "text-[var(--toast-warning-text-color)]" "text-[var(--toast-warning-icon-color)]"
      # "border-[var(--toast-danger-border-color)]" "bg-[var(--toast-danger-background-color)]" "text-[var(--toast-danger-text-color)]" "text-[var(--toast-danger-icon-color)]"
      # "text-[var(--toast-dismiss-text-color)]" "hover:text-[var(--toast-dismiss-hover-text-color)]" "hover:bg-[var(--toast-dismiss-hover-background-color)]" "pointer-events-auto"
      STYLES = {
        info: {
          border: "border-[var(--toast-info-border-color)]",
          bg: "bg-[var(--toast-info-background-color)]",
          text: "text-[var(--toast-info-text-color)]",
          icon: "text-[var(--toast-info-icon-color)]"
        },
        success: {
          border: "border-[var(--toast-success-border-color)]",
          bg: "bg-[var(--toast-success-background-color)]",
          text: "text-[var(--toast-success-text-color)]",
          icon: "text-[var(--toast-success-icon-color)]"
        },
        warning: {
          border: "border-[var(--toast-warning-border-color)]",
          bg: "bg-[var(--toast-warning-background-color)]",
          text: "text-[var(--toast-warning-text-color)]",
          icon: "text-[var(--toast-warning-icon-color)]"
        },
        danger: {
          border: "border-[var(--toast-danger-border-color)]",
          bg: "bg-[var(--toast-danger-background-color)]",
          text: "text-[var(--toast-danger-text-color)]",
          icon: "text-[var(--toast-danger-icon-color)]"
        }
      }.freeze

      # Density for padding, gap, and text. md matches --toast-padding 1rem + text-sm.
      # "gap-2" "gap-3" "gap-4" "text-xs" "text-sm" "text-base" "w-4" "h-4" "w-5" "h-5" "w-6" "h-6"
      SIZES = {
        sm: {padding: "0.75rem", gap: "gap-2", text: "text-xs", icon: "w-4 h-4"},
        md: {padding: "1rem", gap: "gap-3", text: "text-sm", icon: "w-5 h-5"},
        lg: {padding: "1.25rem", gap: "gap-4", text: "text-base", icon: "w-6 h-6"}
      }.freeze

      def initialize(
        text:,
        style: :info,
        timeout: 5000,
        dismissible: true,
        size: :md,
        **system_arguments
      )
        super(**system_arguments)
        @text = text
        @style = style.to_sym
        @timeout = timeout
        @dismissible = dismissible
        @size = size.to_sym

        validate_text!
        validate_style!
        validate_timeout!
        validate_size!
      end

      def call
        content_tag(:div, **toast_attributes) do
          safe_join([
            render_icon,
            render_text,
            render_dismiss_button
          ].compact)
        end
      end

      private

      def size_config
        SIZES.fetch(@size)
      end

      def toast_attributes
        merge_attributes(
          role: "status",
          aria: {live: "polite", atomic: "true"},
          class: toast_classes,
          style: padding_style,
          data: {
            controller: "flat-pack--toast",
            "flat-pack--toast-timeout-value": @timeout,
            "flat-pack--toast-dismissible-value": @dismissible
          }
        )
      end

      def padding_style
        declaration = "--toast-padding: #{size_config.fetch(:padding)}"
        existing = @system_arguments[:style] || @system_arguments["style"]
        return declaration unless existing.present?

        "#{existing.to_s.rstrip.sub(/;+\z/, "")}; #{declaration}"
      end

      def toast_classes
        type_styles = STYLES.fetch(@style)
        classes(
          "flex",
          "items-center",
          "pointer-events-auto",
          size_config.fetch(:gap),
          "p-[var(--toast-padding)]",
          "rounded-[var(--toast-border-radius)]",
          "border",
          "shadow-[var(--toast-shadow)]",
          "z-[60]",
          "min-w-[300px]",
          "max-w-md",
          type_styles[:border],
          type_styles[:bg],
          type_styles[:text]
        )
      end

      def render_icon
        type_styles = STYLES.fetch(@style)
        content_tag(:div, class: "flex-shrink-0 #{type_styles[:icon]}") do
          icon_svg
        end
      end

      def icon_svg
        case @style
        when :success
          success_icon
        when :warning
          warning_icon
        when :danger
          danger_icon
        else
          info_icon
        end
      end

      def icon_class
        size_config.fetch(:icon)
      end

      def info_icon
        content_tag(:svg, class: icon_class, fill: "currentColor", viewBox: "0 0 20 20") do
          tag.path(
            "fill-rule": "evenodd",
            d: "M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z",
            "clip-rule": "evenodd"
          )
        end
      end

      def success_icon
        content_tag(:svg, class: icon_class, fill: "currentColor", viewBox: "0 0 20 20") do
          tag.path(
            "fill-rule": "evenodd",
            d: "M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z",
            "clip-rule": "evenodd"
          )
        end
      end

      def warning_icon
        content_tag(:svg, class: icon_class, fill: "currentColor", viewBox: "0 0 20 20") do
          tag.path(
            "fill-rule": "evenodd",
            d: "M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z",
            "clip-rule": "evenodd"
          )
        end
      end

      def danger_icon
        content_tag(:svg, class: icon_class, fill: "currentColor", viewBox: "0 0 20 20") do
          tag.path(
            "fill-rule": "evenodd",
            d: "M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z",
            "clip-rule": "evenodd"
          )
        end
      end

      def render_text
        content_tag(:p, @text, class: "flex-1 font-medium #{size_config.fetch(:text)}")
      end

      def render_dismiss_button
        return nil unless @dismissible

        content_tag(:button,
          type: "button",
          class: dismiss_button_classes,
          aria: {label: "Dismiss"},
          data: {action: "flat-pack--toast#dismiss"}) do
          render FlatPack::Shared::IconComponent.new(name: "x-mark", size: :sm)
        end
      end

      def dismiss_button_classes
        classes(
          "inline-flex",
          "items-center",
          "justify-center",
          "flex-shrink-0",
          "cursor-pointer",
          "transition-colors",
          "rounded-[var(--radius-sm)]",
          "p-1.5",
          "fp-hit-slop",
          "text-[var(--toast-dismiss-text-color)]",
          "hover:text-[var(--toast-dismiss-hover-text-color)]",
          "hover:bg-[var(--toast-dismiss-hover-background-color)]",
          "focus-visible:outline-none",
          "focus-visible:ring-2 focus-visible:ring-inset",
          "focus-visible:ring-ring"
        )
      end

      def validate_text!
        return if @text.present?
        raise ArgumentError, "text is required"
      end

      def validate_style!
        return if STYLES.key?(@style)
        raise ArgumentError, "Invalid style: #{@style}. Must be one of: #{STYLES.keys.join(", ")}"
      end

      def validate_timeout!
        return if @timeout.is_a?(Integer) && @timeout >= 0
        raise ArgumentError, "timeout must be a non-negative integer"
      end

      def validate_size!
        return if SIZES.key?(@size)
        raise ArgumentError, "Invalid size: #{@size}. Must be one of: #{SIZES.keys.join(", ")}"
      end
    end
  end
end
