...
| `side_preview` | Boolean | `false` | no | In default slide mode, reveals `0.25` of the next slide while preserving configured full items-per-view. Ignored for `:logo_slider` and `:fade`. |
