...
<%= render FlatPack::TopNav::Component.new do |nav| %>
  ...
  <% nav.right do %>
    <button class="p-2 rounded-lg hover:bg-[var(--surface-muted-background-color)]">
...
<% nav.left do %>
  <button
    type="button"
    class="md:hidden p-2 rounded-lg hover:bg-[var(--surface-muted-background-color)]"
...
<%= render FlatPack::TopNav::Component.new do |nav| %>
  ...
  <% nav.right do %>
    <button class="p-2 rounded-lg hover:bg-[var(--surface-muted-background-color)]">Alerts</button>
  <% end %>
<% end %>
```
...
<%= render FlatPack::TopNav::Component.new do |nav| %>
  ...
  <% nav.right do %>
    <button class="p-2 rounded-lg hover:bg-[var(--surface-muted-background-color)]">Save</button>
  <% end %>
<% end %>
```
...
<%= render FlatPack::TopNav::Component.new do |nav| %>
  <% nav.left do %>
    <button
      type="button"
      class="md:hidden p-2 rounded-lg hover:bg-[var(--surface-muted-background-color)] transition-colors"
      data-flat-pack--sidebar-layout-target="mobileToggle"
      data-action="click->flat-pack--sidebar-layout#toggleMobile"
  ...
  <% nav.right do %>
    <div class="flex items-center gap-2">
      ...
      <button class="p-2 rounded-lg hover:bg-[var(--surface-muted-background-color)] transition-colors relative">
        <%= render FlatPack::Shared::IconComponent.new(name: :bell, size: :md) %>
        <span class="absolute top-1 right-1 w-2 h-2 bg-red-500 rounded-full"></span>
      </button>
      
      <button class="p-2 rounded-lg hover:bg-[var(--surface-muted-background-color)] transition-colors">
        <%= render FlatPack::Shared::IconComponent.new(name: :user, size: :md) %>
      </button>
    </div>
  <% end %>
...
<%= render FlatPack::TopNav::Component.new do |nav| %>
  ...
  <% nav.center do %>
    <nav class="flex gap-1 p-1 bg-[var(--surface-muted-background-color)] rounded-lg">
      <%= link_to "All", projects_path, class: "px-4 py-1.5 text-sm font-medium rounded-md #{'bg-[var(--surface-background-color)] shadow-sm' if current_page?(projects_path)}" %>
      <%= link_to "Active", active_projects_path, class: "px-4 py-1.5 text-sm font-medium rounded-md #{'bg-[var(--surface-background-color)] shadow-sm' if current_page?(active_projects_path)}" %>
      <%= link_to "Archived", archived_projects_path, class: "px-4 py-1.5 text-sm font-medium rounded-md #{'bg-[var(--surface-background-color)] shadow-sm' if current_page?(archived_projects_path)}" %>
    </nav>
  <% end %>
...
Uses `backdrop-blur-lg` with semi-transparent background (`bg-[var(--surface-background-color)]/80`) for a modern, elevated appearance.
