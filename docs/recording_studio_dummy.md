...
Cursor resource identity is the MCP URL (`…/recording_studio_mcp`), not API `/recording_studio_api/api`. Cursor may open `GET /authorize` on the tunnel origin; the dummy 302s that path to `/recording_studio_oauth/oauth/authorize` with the query string intact. Cursor may POST `/token` and `/revoke` on the origin; dummy 307s to `/recording_studio_api/oauth/token` and `/recording_studio_api/oauth/revoke` so method and body survive.
...
`list` returns skinny records plus `meta`: `count`, `gem_version` (`FlatPack::VERSION`), and `publicity` (`scope: "public"` with short `excludes` rule strings from the publicity table — not excluded class names). `show` returns the same skinny keys plus `parameters`, `slots`, and `examples`. `examples` copies `erb` and `ruby` fences from the first `## Example` section of the component doc. A fence is stored under each `FlatPack::` class it names. The array is empty when no fence names the class. Each parameter has `name`, `required`, and `type`. It may also have `enum` and `default`. `default` is present when `initialize` uses a JSON-safe literal for that kwarg, including explicit `nil`. The key is omitted when the default is unknown (required, keyrest, a constant, a call, or unreadable source).
