...
When `rich_text: true` is set on `FlatPack::TextArea::Component`, the native `<textarea>` is replaced with a fully featured [TipTap](https://tiptap.dev) editor. The editor is rendered server-side as empty containers and bootstrapped by the `flat-pack--tiptap` Stimulus controller at runtime. The editor surface uses the same `FormField::ControlStyles#form_control_border_classes` as a plain textarea (`border` plus `--surface-border-color`, or `--color-error` when invalid) so host Tailwind preflight cannot wipe the field chrome. Toolbar, bubble, and editor chrome use kit `--radius-md` / `--radius-sm` (with `1rem` / `0.75rem` fallbacks). If host Tailwind loads last, re-set those radii on unlayered `:root` so Tailwind’s `0.375rem` default does not show up as tight editor corners. See [Theming](../theming.md).
