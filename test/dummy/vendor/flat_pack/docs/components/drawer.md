...
- Opening moves the overlay onto `document.body` so `z-[70]` can sit above `SidebarLayout` (`z-50`). A hidden `data-fp-drawer-slot` marker holds the original place. Stimulus reconnect after the move does not restore the overlay while it is still on `document.body`; close puts it back.
