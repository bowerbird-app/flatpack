...
class PagesTimestampDemoTest < ActionDispatch::IntegrationTest
  test "timestamp page renders relative and fallback examples" do
    ...
    assert_includes response.body, "Less than a minute"
    assert_includes response.body, "Weeks"
    assert_includes response.body, "Unshortened"
    assert_includes response.body, "Shortened"
    assert_includes response.body, "20 min ago"
    assert_includes response.body, "2wk ago"
    assert_includes response.body, "In 5hr"
    assert_includes response.body, "data-controller=\"flat-pack--tooltip\""
    assert_includes response.body, "data-flat-pack--timestamp-shorten-timestamp-value=\"true\""
    assert_includes response.body, "Styling Examples"
